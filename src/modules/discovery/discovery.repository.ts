...
export class DiscoveryRepository {
  ...
  private filterWhere(
    ...
    if (dto.minRating) {
      where.push(`f.average_rating >= ${this.addParam(values, dto.minRating)}`);
    }
    ...
    if (dto.cuisineId || dto.cuisineSlug || dto.cuisine) {
      ...
      if (dto.cuisineId) {
        cuisineConditions.push(
          `c2.id = ${this.addParam(values, dto.cuisineId)}::uuid`,
        );
      }

      if (dto.cuisineSlug) {
        cuisineConditions.push(
          `c2.slug = ${this.addParam(values, dto.cuisineSlug)}`,
        );
      }

      if (dto.cuisine) {
        cuisineConditions.push(
          `c2.name ILIKE ${this.addParam(values, `%${dto.cuisine}%`)}`,
        );
      }
