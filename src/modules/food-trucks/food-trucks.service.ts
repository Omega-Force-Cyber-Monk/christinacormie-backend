...
export class FoodTrucksService {
  ...
  async setupBasicMenu(
    userId: string,
    foodTruckId: string,
    dto: SetupBasicMenuDto,
  ) {
    await this.ensureOwnFoodTruck(userId, foodTruckId);
    ...
    for (const category of dto.categories) {
      if (!category.items.length) {
        throw new BadRequestException(
          'Each menu category needs at least one item',
        );
      }
    }
  ...
  async setupServiceArea(
    userId: string,
    foodTruckId: string,
    dto: SetupServiceAreaDto,
  ) {
    await this.ensureOwnFoodTruck(userId, foodTruckId);
  ...
  async createAvailabilityException(
    ...
    await this.ensureOwnFoodTruck(userId, foodTruckId);
    return this.foodTrucksRepository.createAvailabilityException(
      foodTruckId,
      dto,
    );
  }
  ...
  async updateAvailabilityException(
    ...
    await this.ensureOwnFoodTruck(userId, foodTruckId);
    const exception =
      await this.foodTrucksRepository.findAvailabilityExceptionById(
        exceptionId,
      );
    ...
    return this.foodTrucksRepository.updateAvailabilityException(
      exceptionId,
      dto,
    );
  }
  ...
  private async ensureOwnMenuCategory(
    ...
    await this.ensureOwnFoodTruck(userId, foodTruckId);
    const category =
      await this.foodTrucksRepository.findMenuCategoryById(categoryId);
