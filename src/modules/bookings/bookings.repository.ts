...
export class BookingsRepository {
    ...
    const [bookingOverlap, holdOverlap] = await this.prisma.$transaction([
      this.prisma.booking.count({
        where: {
          foodTruckId,
          status: {
            in: ['PAYMENT_PENDING', 'CONFIRMED', 'IN_PROGRESS'] as any,
          },
          startsAt: { lt: endsAt },
  ...
  async createQuote(
    ...
    const outsideRadiusFee = dto.outsideRadiusFee ?? dto.transportFee ?? 0;
    const transportFee = dto.transportFee ?? outsideRadiusFee;
    const baseServiceFee = dto.baseServiceFee ?? dto.subtotal ?? 0;
    const subtotal = dto.subtotal ?? baseServiceFee;
    ...
    const derivedDeposit =
      paymentPreference === 'DEPOSIT_ONLY'
        ...
        : paymentPreference === 'PREPAID_IN_FULL'
          ? totalAmount
          : 0;
    const depositAmount = Math.min(totalAmount, derivedDeposit);
      ...
      const quote = await tx.bookingQuote.create({
        data: {
          ...
          selectedMenuItems: dto.selectedMenuItems
            ? (dto.selectedMenuItems as any)
            : null,
          extraCharges: dto.extraCharges ? (dto.extraCharges as any) : null,
          baseServiceFee,
          transportFee,
          subtotal,
          outsideRadiusFee,
          ...
          discountAmount,
          totalAmount,
          paymentPreference: paymentPreference as any,
          depositAmount,
          depositPercent,
          balanceDueAtEvent,
          message: dto.message ?? null,
          noteToClient: dto.noteToClient ?? null,
